...
const sportStyles=['Lead','Top Rope'];
...
function rememberedSportStyle(){const saved=localStorage.getItem('climbtracker.lastSportStyle');if(sportStyles.includes(saved))return saved;const recent=[...state.climbs].filter(c=>c.type==='Sport'&&sportStyles.includes(c.sportStyle)).sort((a,b)=>b.date-a.date)[0];return recent?.sportStyle||sportStyles[0]}
function rememberSportStyle(style){localStorage.setItem('climbtracker.lastSportStyle',style)}
...
function showClimbForm(id=null,forceProject=false,preferredType=null){
 const c=id?state.climbs.find(x=>x.id===id):null;
 const type=c?.type||preferredType||'Boulder';
 const selectedGrade=c?.grade||rememberedGrade(type);
 const selectedSportStyle=sportStyles.includes(c?.sportStyle)?c.sportStyle:rememberedSportStyle();
 const defaultGym=c?.gym||todayDefaultGym();
 const savedGyms=[...new Set(state.gyms.filter(Boolean))].sort((a,b)=>a.localeCompare(b));
 const gymIsSaved=defaultGym&&savedGyms.some(g=>g.toLowerCase()===defaultGym.toLowerCase());
 openModal(c?'Edit Climb':forceProject?'Add Project':'Log Climb',`<form id="climbForm" class="form-grid"><div class="segmented"><button type="button" data-type="Boulder" class="${type==='Boulder'?'active':''}">Boulder</button><button type="button" data-type="Sport" class="${type==='Sport'?'active':''}">Sport</button></div><div id="sportStyleWrap" class="${type==='Sport'?'':'hidden'}"><label>Sport Style</label><div class="segmented">${sportStyles.map(style=>`<button type="button" data-sport-style="${esc(style)}" class="${style===selectedSportStyle?'active':''}">${esc(style)}</button>`).join('')}</div><input id="sportStyle" type="hidden" value="${esc(selectedSportStyle)}"></div><div><label>Grade</label><div id="gradePicker">${gradeWheelMarkup(type,selectedGrade)}</div></div><div><label>Attempts</label><div class="attempt-stepper"><button type="button" id="attemptMinus" aria-label="Decrease attempts">−</button><output id="attempts">${c?.attempts||1}</output><button type="button" id="attemptPlus" aria-label="Increase attempts">+</button></div></div><label>Gym<select id="gymSelect"><option value="">Select gym</option>${savedGyms.map(g=>`<option value="${esc(g)}" ${gymIsSaved&&g.toLowerCase()===defaultGym.toLowerCase()?'selected':''}>${esc(g)}</option>`).join('')}<option value="__new__" ${defaultGym&&!gymIsSaved?'selected':''}>Add new gym…</option></select></label><label id="newGymWrap" class="${defaultGym&&!gymIsSaved?'':'hidden'}">New Gym<input id="newGym" value="${defaultGym&&!gymIsSaved?esc(defaultGym):''}" autocomplete="off"></label><div><label>Incline</label><div id="inclines" class="chips">${inclines.map(x=>`<button type="button" class="chip incline-chip ${c?.incline===x?'on':''}" data-incline="${esc(x)}">${esc(x)}</button>`).join('')}</div><input id="incline" type="hidden" value="${esc(c?.incline||'')}"></div><div><label>Hold Types</label><div id="holds" class="chips">${chipSet(holds,c?.holdTypes)}</div></div><div><label>Key Moves</label><div id="moves" class="chips">${chipSet(moves,c?.keyMoves)}</div></div>${forceProject?'':`<label class="checkbox-row"><input id="project" type="checkbox" ${c?.isProject?'checked':''}> Mark as Project</label>`}<label>Description / Notes<textarea id="description">${esc(c?.description||'')}</textarea></label><div class="modal-actions climb-form-actions">${c?'<button type="button" id="deleteClimb" class="btn danger">Delete</button>':''}<button type="button" class="btn secondary" onclick="closeModal()">Cancel</button><button class="btn">Save</button></div></form>`,()=>{
  const form=document.querySelector('#climbForm');
  let currentType=type,attemptCount=Math.max(1,c?.attempts||1);
  const attemptOut=document.querySelector('#attempts');
  const setAttempts=n=>{attemptCount=Math.max(1,n);attemptOut.textContent=attemptCount};
  const bindAttemptButton=(button,delta)=>{let holdTimer=null,repeatTimer=null,held=false;const stop=()=>{held=false;clearTimeout(holdTimer);clearInterval(repeatTimer);holdTimer=repeatTimer=null};button.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse'&&e.button!==0)return;e.preventDefault();held=true;setAttempts(attemptCount+delta);try{button.setPointerCapture(e.pointerId)}catch(_){};holdTimer=setTimeout(()=>{if(!held)return;repeatTimer=setInterval(()=>setAttempts(attemptCount+delta),120)},350)});button.addEventListener('pointerup',stop);button.addEventListener('pointercancel',stop);button.addEventListener('lostpointercapture',stop);button.addEventListener('contextmenu',e=>e.preventDefault());button.addEventListener('dblclick',e=>e.preventDefault())};
  bindAttemptButton(document.querySelector('#attemptMinus'),-1);
  bindAttemptButton(document.querySelector('#attemptPlus'),1);
  const gymSelect=document.querySelector('#gymSelect'),newGymWrap=document.querySelector('#newGymWrap'),newGym=document.querySelector('#newGym');
  gymSelect.onchange=()=>{const adding=gymSelect.value==='__new__';newGymWrap.classList.toggle('hidden',!adding);if(adding)setTimeout(()=>newGym.focus(),0)};
  const sportStyleWrap=document.querySelector('#sportStyleWrap'),sportStyleInput=document.querySelector('#sportStyle');
  form.querySelectorAll('[data-sport-style]').forEach(btn=>btn.onclick=()=>{sportStyleInput.value=btn.dataset.sportStyle;form.querySelectorAll('[data-sport-style]').forEach(x=>x.classList.toggle('active',x===btn))});
  setupGradeWheel(currentType,selectedGrade);
  form.querySelectorAll('[data-type]').forEach(btn=>btn.onclick=()=>{currentType=btn.dataset.type;form.querySelectorAll('[data-type]').forEach(x=>x.classList.toggle('active',x===btn));sportStyleWrap.classList.toggle('hidden',currentType!=='Sport');const picked=c&&c.type===currentType?c.grade:rememberedGrade(currentType);document.querySelector('#gradePicker').innerHTML=gradeWheelMarkup(currentType,picked);setupGradeWheel(currentType,picked)});
  const inclineInput=document.querySelector('#incline');
  form.querySelectorAll('.incline-chip').forEach(ch=>ch.onclick=()=>{inclineInput.value=ch.dataset.incline;form.querySelectorAll('.incline-chip').forEach(x=>x.classList.toggle('on',x===ch))});
  form.querySelectorAll('#holds .chip,#moves .chip').forEach(ch=>ch.onclick=()=>ch.classList.toggle('on'));
  form.onsubmit=e=>{e.preventDefault();const gym=(gymSelect.value==='__new__'?newGym.value:gymSelect.value).trim()||null;const val={id:c?.id||uid(),grade:document.querySelector('#grade').value,type:currentType,sportStyle:currentType==='Sport'?sportStyleInput.value:null,date:c?.date||Date.now(),gym,incline:document.querySelector('#incline').value||null,holdTypes:selectedChips('#holds'),keyMoves:selectedChips('#moves'),description:document.querySelector('#description').value.trim()||null,attempts:attemptCount,isProject:forceProject?true:document.querySelector('#project')?.checked||false};val.isFlash=val.attempts===1&&!val.isProject;rememberGrade(val.type,val.grade);if(val.sportStyle)rememberSportStyle(val.sportStyle);if(c)state.climbs=state.climbs.map(x=>x.id===c.id?val:x);else state.climbs.push(val);if(val.gym&&!state.gyms.some(g=>g.toLowerCase()===val.gym.toLowerCase()))state.gyms.push(val.gym);closeModal();save()};
  if(c)document.querySelector('#deleteClimb').onclick=()=>{if(confirm('Delete this climb?')){state.climbs=state.climbs.filter(x=>x.id!==c.id);closeModal();save()}}
 })
}
...
function projectGroups(){const projects=state.climbs.filter(c=>c.isProject&&c.type===projectsType).sort((a,b)=>b.date-a.date),map=new Map();projects.forEach(c=>{const gym=(c.gym||'No Gym').trim()||'No Gym',key=gym.toLowerCase();if(!map.has(key))map.set(key,{gym,climbs:[]});map.get(key).climbs.push(c)});return [...map.values()].sort((a,b)=>a.gym.localeCompare(b.gym))}
function renderProjects(){const groups=projectGroups();app.innerHTML=`<div class="stack"><div class="segmented project-type-tabs"><button onclick="projectsType='Boulder';renderProjects()" class="${projectsType==='Boulder'?'active':''}">Boulder</button><button onclick="projectsType='Sport';renderProjects()" class="${projectsType==='Sport'?'active':''}">Sport</button></div>${groups.length?groups.map(projectGymCard).join(''):`<div class="empty">No active ${projectsType.toLowerCase()} projects.</div>`}</div><button class="fab" onclick="showClimbForm(null,true,'${projectsType}')">+</button>`;setupClimbSwipeDelete()}
function projectGymCard(group){const {gym,climbs:items}=group;return `<details class="card session-card project-gym-card"><summary class="session-summary"><div class="project-gym-summary-text"><strong>${esc(gym)}</strong><div class="muted">${items.length} project${items.length===1?'':'s'}</div></div><div class="session-summary-right"><span class="session-chevron">⌄</span></div></summary><div class="project-gym-climbs">${items.map(projectCard).join('')}</div></details>`}
function projectCard(c){const sportStyleTag=c.type==='Sport'&&c.sportStyle?` <span class="pill">${esc(c.sportStyle)}</span>`:'';return `<div class="swipe-delete-row project-swipe-row" data-climb-id="${c.id}"><button class="swipe-delete-action" type="button" aria-label="Delete ${displayGrade(c)} project">Delete</button><div class="card project-card clickable swipe-delete-content" data-project-id="${c.id}" role="button" tabindex="0" aria-label="Edit ${displayGrade(c)} project" onkeydown="if((event.key==='Enter'||event.key===' ')&&!event.target.closest('button')){event.preventDefault();showClimbForm(${c.id})}"><button type="button" class="project-send-btn" aria-label="Log project as sent" onclick="event.stopPropagation();sendProject(${c.id})">✓</button><div class="project-card-main"><div class="grade">${displayGrade(c)}</div><div class="muted project-type-label">${esc(c.type)}${sportStyleTag}</div><div class="muted project-attempt-count">Attempts: ${c.attempts}</div>${c.description?`<div class="project-description">${esc(c.description)}</div>`:''}</div><div class="project-card-actions"><button type="button" class="btn secondary project-attempt-btn" onclick="event.stopPropagation();projectAttempt(${c.id})">+1 Attempt</button></div></div></div>`}
function projectAttempt(id){const c=state.climbs.find(x=>x.id===id);if(!c)return;c.attempts++;localStorage.setItem(STORE,JSON.stringify(state));const card=document.querySelector(`.project-card[data-project-id="${id}"]`),count=card?.querySelector('.project-attempt-count');if(count)count.textContent=`Attempts: ${c.attempts}`}
...
function bodyWeightRegression(days=30){
  const end=dayStart(),start=end-(days-1)*86400000,dayMs=86400000;
  const points=state.logs.filter(l=>{const d=dayStart(new Date(l.date)),v=l.weight;return d>=start&&d<=end&&v!=null&&Number.isFinite(Number(v))}).map(l=>({date:dayStart(new Date(l.date)),value:Number(l.weight)})).sort((a,b)=>a.date-b.date);
  if(points.length<2)return null;
  const xs=points.map(p=>(p.date-start)/dayMs),ys=points.map(p=>p.value),xMean=xs.reduce((a,b)=>a+b,0)/xs.length,yMean=ys.reduce((a,b)=>a+b,0)/ys.length;
  const denom=xs.reduce((sum,x)=>sum+(x-xMean)**2,0);
  if(!denom)return null;
  const slope=xs.reduce((sum,x,i)=>sum+(x-xMean)*(ys[i]-yMean),0)/denom,intercept=yMean-slope*xMean;
  return {slopePerDay:slope,slopePerWeek:slope*7,startValue:intercept,endValue:intercept+slope*(days-1)};
}
function bodyWeightTrendLabel(){
  const regression=bodyWeightRegression(30);
  if(!regression)return '—';
  const weekly=Math.abs(regression.slopePerWeek)<0.05?0:regression.slopePerWeek;
  return `${weekly>0?'+':''}${weekly.toFixed(1)} lbs per week`;
}
function bodyScatterPlot(field,unit){
  ...
  const scaleValues=points.map(p=>p.value);if(regression)scaleValues.push(regression.startValue,regression.endValue);
  ...
  const trendLine=regression?`<line x1="${x(start).toFixed(1)}" y1="${y(regression.startValue).toFixed(1)}" x2="${x(end).toFixed(1)}" y2="${y(regression.endValue).toFixed(1)}" class="chart-series-line" style="stroke-dasharray:6 6;opacity:.8"/>`:'';
  ...
  return `<div class="interactive-chart"><svg class="scatter-chart" viewBox="0 0 ${W} ${H}" role="img">${grid.join('')}<line x1="${pad.l}" y1="${pad.t+plotH}" x2="${W-pad.r}" y2="${pad.t+plotH}" class="chart-axis"/>${xLabels.join('')}${trendLine}${line}${dots}</svg><div class="chart-tooltip" hidden></div></div>`;
}
...
function buildCsv(){let s='[CLIMBS]\ngrade,type,date,gym,incline,holdTypes,keyMoves,description,attempts,isFlash,isProject,sportStyle\n';state.climbs.forEach(c=>s+=[c.grade,c.type,formatCsvDate(c.date),c.gym,c.incline,c.holdTypes,c.keyMoves,c.description,c.attempts,c.isFlash,c.isProject,c.sportStyle].map(csvEscape).join(',')+'\n');if(state.logs.length){s+='\n[LOGS]\ndate,calories,weight,calorieHistory\n';state.logs.forEach(l=>s+=[formatCsvDate(l.date),l.calories,l.weight,l.calorieHistory].map(csvEscape).join(',')+'\n')}return s}
