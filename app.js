...
function projectCard(c){return `<div class="card project-card clickable" data-project-id="${c.id}" role="button" tabindex="0" aria-label="Edit ${displayGrade(c)} project" onclick="if(!event.target.closest('button'))showClimbForm(${c.id})" onkeydown="if((event.key==='Enter'||event.key===' ')&&!event.target.closest('button')){event.preventDefault();showClimbForm(${c.id})}"><div class="row between"><div><div class="grade">${displayGrade(c)}</div><div class="muted">${esc(c.incline||c.type)}</div></div><strong class="project-attempt-count">${c.attempts} attempt${c.attempts===1?'':'s'}</strong></div>${c.description?`<div>${esc(c.description)}</div>`:''}<div class="toolbar"><button type="button" class="btn secondary" onclick="event.stopPropagation();projectAttempt(${c.id})">+1 Attempt</button><button type="button" class="btn" onclick="event.stopPropagation();sendProject(${c.id})">Sent ✓</button></div></div>`}
function projectAttempt(id){const c=state.climbs.find(x=>x.id===id);if(!c)return;c.attempts++;localStorage.setItem(STORE,JSON.stringify(state));const card=document.querySelector(`.project-card[data-project-id="${id}"]`),count=card?.querySelector('.project-attempt-count');if(count)count.textContent=`${c.attempts} attempt${c.attempts===1?'':'s'}`}
